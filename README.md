...
- 권리 근거와 사용 범위가 확인되지 않은 밈의 다운로드·삽입을 차단합니다.
...
> 템플릿명은 의미 설명을 위한 식별자일 뿐 이용허락을 뜻하지 않습니다. 실제 삽입에는 별도의 권리 근거와 사용 범위 확인이 필요합니다.
...
- 원출처 또는 법적 이용 근거를 확인하지 못한 후보는 내부 발표를 포함해 `provisional` 상태에 머물며 다운로드·복사·삽입되지 않습니다.
...
권리 근거와 사용 범위가 확인되면 스킬은 제공된 원본 파일을 보존하고, 최종 덱의 `assets/memes/` 아래에 ASCII 파일명으로 복사하거나 단일 HTML 요청 시 Base64로 포함합니다. 권리 정보가 없으면 `user-provided-unverified`와 `provisional`로 기록하며 내부 발표에도 복사하지 않습니다.
...
4. 라이선스·허락·퍼블릭도메인 또는 적용 법령상 예외 근거를 확인합니다.
...
스킬은 사용 범위와 근거 필드의 존재를 감사하지만 법률 의견이나 법원의 판단을 대신하지 않습니다. 조문상 예외를 사용하는 경우 발표 목적, 필요 분량, 주종관계, 시장 영향과 가시적 출처를 기록합니다.
...
- 이미지 출처만으로 허락을 추정하지 않으며 내부 발표에도 법적 근거를 요구합니다.
