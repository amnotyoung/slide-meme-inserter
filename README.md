...
내부 교육 자료라고 명시된 경우에는 공식 방송사·출판사·제작자 페이지의 원본 영상에서 필요한 한 장면을 캡처하는 방식도 허용합니다.

- 공식 원본 URL과 영상 시점을 기록합니다.
- 로고·워터마크·크레딧을 제거하지 않습니다.
- `reuse_status: rights unclear`, `distribution_limit: internal only`로 기록합니다.
- 로그인 제한, 유료벽, DRM 또는 다운로드 제한을 우회하지 않습니다.
- 발표 녹화본, PDF, HTML 파일과 이후 온라인 업로드도 내부 배포 범위로 제한합니다.
- 외부 공개 전에는 허가된 이미지로 교체하거나 별도 사용 허가를 확인합니다.
